...
locateMPDConf() {
  ...
  if [[ -z $config ]]; then
    if [[ -r "$HOME/.mpd/config" ]]; then
      config="$HOME/.mpd/config"
    elif [[ -r '/etc/mpd.conf' ]]; then
      config='/etc/mpd.conf'
    else
      errorout "unable to locate a readable mpd.conf"
    fi
  fi
...
touch "$list" 2>/dev/null || errorout 'unable to write playlist file'
